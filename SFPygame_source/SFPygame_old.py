...
def get_palette(palette_file):
    ...
    palette=[[0,0,0]for i in range(265)]
...
def load_RE(RE,IDE,PALETTE):
    ...
    frames=[]
    len_collisions=len(collisions)-1
...
def load_KEY(KEY):
    ...
    for i in range(len(throws)):
        for j in range(0,len(throws[i])-1,1):
            if j != 1:
               throws[i][j]=int(throws[i][j])
        if len(throws[i])==6:
           throws[i][0:0]=[4]
        throw_data=throws[i]
        throw={'damage':throw_data[0],'throw_height':throw_data[1],
               'direction_held':int(throw_data[2][0]),
               'button_held':int(throw_data[2][1]),
               'character_sequence':throw_data[3],
               'opponent_sequence':throw_data[4],
               'slam_direction':throw_data[5],'sound':throw_data[6]}
        throws[i]=throw
...
def load_background(image_path,palette):
    ...
    image_width=int(image_path.split('.')[1])
...
sfibm_path="SFIBM 125\\"
IDE=sfibm_path+"RYU.IDE"
RE=sfibm_path+"RYU.RE"
SEQ=sfibm_path+"RYU.SEQ"
KEY=sfibm_path+"RYU.KEY"
IDE2=sfibm_path+"KEN.IDE"
RE2=sfibm_path+"KEN.RE"
SEQ2=sfibm_path+"KEN3.SEQ"
KEY2=sfibm_path+"KEN.KEY"
background_path=sfibm_path+"RBK.484"
    ...
    def punch(self):
        ...
        if character.current_sequence_frame['cancel_mode']>1:
           ...
           if character.state=='stand':
              character.sequence_index=-1
              character.anim_time=character.max_anim_time
              if attack_range<=character.close_range and character.opponent.state!='jump':
...
def character_select(player1_controls, player2_controls):
    ...
    characters_faces.set_palette(PALETTE)
    world_map.set_colorkey(alpha_color)
    characters_faces.set_colorkey(alpha_color)
    world_map.convert()
    characters_faces.convert()
    ...
    player1_selection_box_pos = [characters_faces_pos[0], characters_faces_pos[1]-8]
    selection_box_x_move = player1_selection_box.get_width() - 3
    selection_box_y_move = player1_selection_box.get_height() - 12
    box1_flash_time = 0
    box2_flash_time = 0
    box_flash_surface = pygame.Surface((player1_selection_box.get_width(), player1_selection_box.get_height()-8))
    box_flash_surface.fill(WHITE)
    box_flash_surface.set_alpha(100)
    player2_selection_box = images[13]['image']
    player2_selection_box_pos = [characters_faces_pos[0], characters_faces_pos[1]-8+selection_box_y_move]
    characters = [[{'name':'RYU', 'portrait':images[0]['image'], 'RE':'RYU.RE', 'IDE':'RYU.IDE', 'SEQ':'RYU.SEQ', 'KEY':'RYU.KEY','background':'RBK.484'},
                   {'name':'E.HONDA', 'portrait':images[1]['image'], 'RE':'HONDA.RE', 'IDE':'HONDA.IDE', 'SEQ':'HONDA.SEQ', 'KEY':'HONDA.KEY','background':'HBK.452'},
                   {'name':'BLANKA', 'portrait':images[2]['image'], 'RE':'BLANKA.RE', 'IDE':'BLANKA.IDE', 'SEQ':'BLANKA.SEQ', 'KEY':'BLANKA.KEY','background':'BBK.492'},
                   {'name':'GUILE', 'portrait':images[3]['image'], 'RE':'GUILE.RE', 'IDE':'GUILE.IDE', 'SEQ':'GUILE.SEQ', 'KEY':'GUILE.KEY','background':'GBK.472'}],
                  [{'name':'KEN', 'portrait':images[4]['image'], 'RE':'KEN.RE', 'IDE':'KEN.IDE', 'SEQ':'KEN3.SEQ', 'KEY':'KEN.KEY','background':'KBK.452'},
                   {'name':'CHUN LI', 'portrait':images[5]['image'], 'RE':'CHUNLI.RE', 'IDE':'CHUNLI.IDE', 'SEQ':'CHUNLI.SEQ', 'KEY':'CHUNLI.KEY','background':'CBK.452'},
                   {'name':'ZANGIEF', 'portrait':images[6]['image'], 'RE':'ZANGIEF.RE', 'IDE':'ZANGIEF.IDE', 'SEQ':'ZANGIEF.SEQ', 'KEY':'ZANGIEF.KEY','background':'ZBK.432'},
                   {'name':'DHALSIM', 'portrait':images[7]['image'], 'RE':'DALSIM.RE', 'IDE':'DALSIM@.IDE', 'SEQ':'DALSIM2.SEQ', 'KEY':'DALSIM2.KEY','background':'DBK.452'}]]
    player1_column_index = 0
    player1_row_index = 0
    player1_character = characters[player1_row_index][player1_column_index]
    player1_character_pos =[10, 240]
    player2_column_index = 0
    player2_row_index = 1
    player2_character = characters[player2_row_index][player2_column_index]
    player2_character_pos =[640-player2_character['portrait'].get_width()-10, 240]
    player1_choice = None
    player2_choice = None
    character1 = None
    character2 = None
    ...
    while True:
                  ...
                  if event.key == K_RIGHT:
                     ...
                     if player1_column_index >= len(characters[player1_row_index]):
                        ...
                        if player1_row_index >= len(characters):
                           player1_row_index = 0
                        player1_selection_box_pos = [characters_faces_pos[0],
                                                    (characters_faces_pos[1]-8)+(selection_box_y_move*player1_row_index)]
                     player1_character = characters[player1_row_index][player1_column_index]
                  elif event.key == K_LEFT:
                     player1_selection_box_pos[0] -= selection_box_x_move
                     player1_column_index -= 1
                     if player1_column_index < 0:
                        ...
                        if player1_row_index >= len(characters):
                           player1_row_index = 0
                        player1_selection_box_pos = [characters_faces_pos[0]+(selection_box_x_move*(len(characters[player1_row_index])-1)),
                                                    (characters_faces_pos[1]-8)+(selection_box_y_move*player1_row_index)]
                     player1_character = characters[player1_row_index][player1_column_index]
                  elif event.key == K_UP:
                     ...
                     if player1_row_index < 0:
                        ...
                        player1_selection_box_pos[1] = (characters_faces_pos[1]-8)+(selection_box_y_move*player1_row_index)
                     player1_character = characters[player1_row_index][player1_column_index]
                     ...
                     if event.key == K_SEMICOLON:
                        ...
                        if player2_column_index >= len(characters[player2_row_index]):
                           ...
                           if player2_row_index >= len(characters):
                              player2_row_index = 0
                           player2_selection_box_pos = [characters_faces_pos[0],
                                                       (characters_faces_pos[1]-8)+(selection_box_y_move*player2_row_index)]
                        player2_character = characters[player2_row_index][player2_column_index]
                     elif event.key == K_l:
                        player2_selection_box_pos[0] -= selection_box_x_move
                        player2_column_index -= 1
                        if player2_column_index < 0:
                           ...
                           if player2_row_index >= len(characters):
                              player2_row_index = 0
                           player2_selection_box_pos = [characters_faces_pos[0]+(selection_box_x_move*(len(characters[player2_row_index])-1)),
                                                       (characters_faces_pos[1]-8)+(selection_box_y_move*player2_row_index)]
                        player2_character = characters[player2_row_index][player2_column_index]
                     elif event.key == K_p:
                        ...
                        if player2_row_index < 0:
                           ...
                           player2_selection_box_pos[1] = (characters_faces_pos[1]-8)+(selection_box_y_move*player2_row_index)
                        player2_character = characters[player2_row_index][player2_column_index]
        ...
        if mode == 'arcade':
           ...
           if character1 and character2:
              ...
              return character1, character2, background, versus_data
        ...
        if mode in ('versus', 'watch'):
           if character2:
              ...
              if box2_flash_time < 5 :
                 screen.blit(box_flash_surface, (player2_selection_box_pos[0],player2_selection_box_pos[1]+8))
        ...
        if character1:
           ...
           if box1_flash_time < 5 :
              screen.blit(box_flash_surface, (player1_selection_box_pos[0],player1_selection_box_pos[1]+8))
...
def versus_screen(versus_data):
    ...
    portrait_x_offset = int(((640-versus_image.get_width())/2 - character1_portrait.get_width())/2)
...
def round_start(character1, character2, round_number):
    ...
    while True:
        ...
        timer += 1
        if timer == 25:
           text1 = font.render('FIGHT !', True, YELLOW)
           text2 = font.render('FIGHT !', True, RED)
           text_pos = [320-int(text1.get_width()/2), 200]
        elif timer == 55:
           return
        ...
        shadow_pos=[character1.axis_pos[0]+character1.sprites[0]['x_axis_shift'],FLOOR_Y_POS-6]
        display_surface.blit(character1.sprites[0]['image'],shadow_pos)
        shadow_pos=[character2.axis_pos[0]+character2.sprites[0]['x_axis_shift'],FLOOR_Y_POS-6]
        ...
        if mode == 'arcade':
           if character1:
              ...
              if len(opponents) > 0 and not character1.knocked_out:
                 data = versus_data[0]
                 character1 = load_character(data['RE'], data['IDE'], data['SEQ'], data['KEY'], controls1, 'left', PALETTE)
                 ...
                 background=load_background(background,PALETTE)
                 character2 = load_character(character2['RE'], character2['IDE'], character2['SEQ'], character2['KEY'], controls2, 'right', PALETTE)
              else:
                 mode ='title screen'
                 character1 = None
                 character2 = None
