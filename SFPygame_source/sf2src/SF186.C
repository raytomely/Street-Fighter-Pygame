...
char **argv ;
{
   ...
   if (argc>1) loadFileConfig(argv[1]) ;
       else    loadFileConfig("sf2file.cfg") ;
   ...
   for ( ; ; )
   {
     demo(&GENV,&LP,&RP) ; offMusic() ;
     for ( ; ; )
     {
       if (select(&GENV,&LP,&RP)==BREAK) break ;
       if (game(&GENV,&LP,&RP)==BREAK) break ;
     }
   }
   systemDown() ;
}
...
char fn[] ;
{
   ...
   fp=fopen(fn,"rb") ;
   if (fp==NULL) cError(fn," File not Found",0) ;
...
PLAYER *LP,*RP ;
{
   LP->K = &LK ; LP->KB = &LKB ; LP->KR = LKR ; LP->CR = LCR ;
   RP->K = &RK ; RP->KB = &RKB ; RP->KR = RKR ; RP->CR = RCR ;

   LP->AN = &ANL ; LP->AR = ARL ; LP->Spr = SprL ; LP->Img = ImgL ;
   RP->AN = &ANR ; RP->AR = ARR ; RP->Spr = SprR ; RP->Img = ImgR ;
...
viewWarn()
{
   ...
   if (fp==NULL) return(0) ;
   p = malloc(64000) ;
   if (p==NULL) cError("","Memory Error",1) ;

   viewPage(0) ; actPage(0) ; pClrScr(0) ;
   fread(p,320,200,fp) ; fclose(fp) ;
...
PLAYER *LP,*RP ;
{
   ...
   loadDemoResource(info) ; playMusic("sf2ttl.mdi") ;
   _KBHIT=0 ;
   for (  ; ; )
   {
     r=CONTINUE ;
    ...
    if (gameSelect(GENV,LP,RP)==CONTINUE) return(CONTINUE) ;
   }
}
...
char info[][80] ;
{
   ...
   if (ImgL[20].p!=NULL) free(ImgL[20].p) ;
   ImgL[20].p=malloc(29999) ; fp=fopen("title.229","rb") ;
   if (ImgL[20].p==NULL) cError("","Demo Memory Error",1) ;
   fread(ImgL[20].p,229,131,fp) ; fclose(fp) ;

   planeWrite0 ; pClrScr(0) ; viewPage(0)  ; actPage(3)  ; pClrScr(0) ;

   loadImgSpr("facew.id",&ImgL,&SprL,0) ;
   pCTrpSpr0(115,101,&SprL[0],&ImgL) ; pCTrpSpr0(200,156,&SprL[3],&ImgL)  ;
   pCTrpSpr0(115,156,&SprL[4],&ImgL) ; pCTrpSpr0(40,156,&SprL[1],&ImgL)   ;
   pCTrpSpr0(235,196,&SprL[7],&ImgL) ; pCTrpSpr0(160,196,&SprL[6],&ImgL) ;
   pCTrpSpr0(85 ,196,&SprL[5],&ImgL) ; pCTrpSpr0(10 ,196,&SprL[2],&ImgL) ;
   return(0) ;
}
...
demo1()
{
   ...
   fadeOut() ; viewPage(3) ; fadeIn() ; page2PageCp(3,0) ;

   vp=0 ; y=190 ; writeMode(0) ;

   for ( i=0 ; i<80 ; i++ )
   {
     viewPage(vp) ; vp = (vp+1)%2 ; actPage(vp) ;
     writeMode(1) ; page2PageCp(3,vp)  ;
     writeMode(0) ; pCTrpRImg0(50,y,ImgL[20].p,229,131) ;
     vy = (i/25)+1; y -= vy      ;
     if (_KE[_ESC]) systemDown() ;
     if (_KBHIT) { _KBHIT=0 ; break ; }
   }
   ...
   for ( i=0 ; i<30 ; i++ )
   {
     ...
     writeMode(1) ; page2PageCp(3,vp)  ;
     writeMode(0) ; pCTrpRImg0(50,y,ImgL[20].p,229,131) ;
     if ((i/5)%2) fPText(100,160,"JF@AH@LT@    AGAQN@AA@DSH",0,0xe0,0xe0) ;
     viewPage(vp) ;
     if (_KE[_ESC]) systemDown() ;
     if (_KBHIT) { _KBHIT=0 ; return(BREAK) ; }
   }
   return(CONTINUE) ;
}
...
char info[][80] ;
{
   ...
   fadeOut() ; actPage(2) ; pClrScr(0) ;
   writeMode(0) ; pCTrpSpr0(15,111,Spr,&ImgL) ;
...
PLAYER *LP,*RP ;
{
   ...
   page2PageCp(3,0) ; actPage(0) ;
   writeMode(0) ; pCTrpRImg0(50,14,ImgL[20].p,229,131) ;
   page2PageCp(0,2) ; writeMode(0) ;
   ...
   LP->pl = NOTSELECT ; LP->ctrl = HUMAN ;
   RP->pl = NOTSELECT ; RP->ctrl = HUMAN ;

   p=0 ; s=1 ;

   flushKey(LP->K) ; flushKey(RP->K) ;

   for ( ; ; )
   {
     viewPage(p) ; p = (p+1)%2 ; actPage(p) ;
     ...
     if (_KE[K->L]) { _KE[K->L]=0 ; s-- ; }
     if (_KE[K->R]) { _KE[K->R]=0 ; s++ ; }
...
//     if (s==0) s++ ;
     ...
     if (_KE[_ESC]) { _KE[_ESC]=0 ; return(EXIT) ; }
   }

   ENV->tr=0 ;
   LP->pl = NOTSELECT ; LP->ctrl = COMPU ;
   RP->pl = NOTSELECT ; RP->ctrl = COMPU ;
   LP->win = RP->win = LP->scr = RP->scr = ENV->mn = 0 ;
   for ( p=0 ; p<12 ; p++ ) ENV->loser[p]=0 ;
   if (s==0) RP->ctrl = HUMAN ;
   if (s==1) { LP->ctrl = HUMAN ; RP->ctrl = HUMAN ; }
   return(CONTINUE) ;
}
...
PLAYER *LP,*RP ;
{
   ...
   K=RP->K ;
   n=ENV->n ; t=ENV->t ;
   lp=LP->power ; lr=LP->resis ;
   rp=RP->power ; rr=RP->resis ;
   ...
   {
     viewPage(p) ; p=(p+1)%2 ;  actPage(p) ;
     page2PageCp(2,p) ; writeMode(0) ;

     if (_KE[K->U]) { _KE[K->U]=0 ; v-- ; }
     if (_KE[K->D]) { _KE[K->D]=0 ; v++ ; }
     if (v<0) v=7 ;
     if (v>7) v=0 ;
//     if (bl) pBarL(x[v],y[v],9,5,255) ;
     if (bl) pCTrpSpr0(x[v],y[v],&SprL[10],&ImgL) ;
     bl = (bl+1)%2 ;

     switch(v)
     {
       case 0 : macroEnv(K->R,K->L,&lp,1,100,1) ; break ;
       case 1 : macroEnv(K->R,K->L,&lr,1,100,1) ; break ;
       case 2 : macroEnv(K->L,K->R,&rp,1,100,1) ; break ;
       case 3 : macroEnv(K->L,K->R,&rr,1,100,1) ; break ;
       case 4 : macroEnv(K->L,K->R,&n,1,99,2)   ; break ;
       case 5 : macroEnv(K->L,K->R,&t,20,999,1) ; break ;
     }
     ...
     {
       flushKey(K) ;
       if (v==7) return(CONTINUE) ;
       if (v==6) break ;
     }
   }
   ENV->n=n ; ENV->t=t ;
   LP->power=lp ; LP->resis=lr ;
   RP->power=rp ; RP->resis=rr ;
   return(0) ;
}
...
drawOpt()
{
   ...
   actPage(0) ; writeMode(0) ; pClrScr(0) ;
...
GameENV *ENV ;
{
   ...
   p1=malloc(195*93) ; p2=malloc(122*72) ;
   if (p1==NULL ||p2==NULL) cError("","Select Memory Error",1) ;

   fp=fopen("select.sf2","rb") ; fread(p1,195,93,fp) ; fread(p2,122,72,fp) ; fclose(fp) ;

   planeWrite0 ; pClrScr(7) ;
...
PLAYER *LP,*RP ;
{
   ...
   if (ENV->tr>7) return(BREAK) ;

   fadeOut() ; writeMode(1) ; page2PageCp(3,0) ; viewPage(0) ; fadeIn() ;

   xL=xR=0 ; yL=0 ; yR=1 ; vp=0  ;
   flushKey(LP->K) ; flushKey(RP->K) ;
   if (LP->pl!=NOTSELECT) { xL=(LP->pl)%4 ; yL=(LP->pl)/4 ; }
   if (RP->pl!=NOTSELECT) { xR=(RP->pl)%4 ; yR=(RP->pl)/4 ; }
...
int *x,*y ;
{
   ...
   {
     if (_KE[K->U]) { ty-- ; _KE[K->U]=0 ; }
     if (_KE[K->D]) { ty++ ; _KE[K->D]=0 ; }
     if (_KE[K->L]) { tx-- ; _KE[K->L]=0 ; }
     if (_KE[K->R]) { tx++ ; _KE[K->R]=0 ; }
     if (tx<0) { ty-- ; tx=3 ; }
     if (tx>3) { ty++ ; tx=0 ; }
     if (ty<0) ty=1 ;
     if (ty>1) ty=0 ;
  ...
  if (PL->pl==NOTSELECT && PL->ctrl==COMPU)
  {
    for ( i=0 ; ; i++ )
    {
      tx=random(4) ; ty=random(2) ;
      if (ENV->loser[ty*4+tx]==0) break ;
    }
    if (random(20)==0) PL->pl = ty*4 + tx ;
  }
...
int x1,y1,x2,y2,p ;
{
   ...
   {
     tn=abs((dy*16)/dx) ;
     if (tn<6)  { m=3 ; if (dx>0) m=6 ; }
     if (tn>38) { m=1 ; if (dy>0) m=2 ; }
     if (tn>5 && tn<39)
     {
       if (dx<0) m += 3 ; if (dx>0) m += 6 ;
       if (dy<0) m += 1 ; if (dy>0) m += 2 ;
     }
   }
   ...
   {
     viewPage(p) ; p=(p+1)%2 ; actPage(p) ;
     writeMode(1) ; page2PageCp(3,p) ;
     writeMode(0) ; pCTrpSpr0(x>>5,y>>5,&SprR[m+17],&ImgR) ;
     x += dx ; y += dy ;
  }
   return(0) ;
}
...
PLAYER *LP,*RP ;
{
   ...
   actPage(0) ; pClrScr(7) ; writeMode(0) ;
...
PLAYER *LP,*RP ;
{
   ...
   {
     GENV->mn=i+1 ;
     if ((r=playGame(GENV,LP,RP))!=CONTINUE) { offMusic() ; return(r) ; }
     if (LP->energe>RP->energe) LP->win++ ;
     if (LP->energe<RP->energe) RP->win++ ;
     if (LP->win*2>GENV->n || RP->win*2>GENV->n) break ;
   }
...
PLAYER *LP,*RP ;
{
   ...
   if (LP->win>RP->win)
   {
     ...
     lp=RP->pl ; lpc=RP->ctrl ; RP->pl=NOTSELECT ;
   }
   else
   {
     pCTrpSpr0( 30,160,&SprR[LP->pl],&ImgR) ;
     pCTrpSpr1(290,160,&SprL[RP->pl],&ImgL) ;
     lp=LP->pl ; lpc=LP->ctrl ; LP->pl=NOTSELECT ;
   }
   
   fPText(44,180,"$You Must Defeat ChengLong !",0,255,255) ;
   viewPage(0) ; fadeIn() ; delay(2000) ;
   r=CONTINUE ;
   if (lpc!=COMPU) r=drawLoser(lp) ;
       else      {  ENV->tr++ ; ENV->loser[lp]=1 ; }
   return(r) ;
}
...
drawLoser(int pl)
{
   ...
   fadeOut() ;
   actPage(0) ; pClrScr(6) ; writeMode(0) ;
...
PLAYER *LP,*RP ;
{
   ...
   if (LP->pl==RP->pl) convertCharacterColor(RP->Img,MAXIMG) ;
   return(0) ;
}
...
int n ;
{
   ...
   for ( i=0 ; i<n ; i++ )
   if (Img[i].p!=NULL) ConvertImageColor(Img[i].p,Img[i].ls,lookup) ;
   return(0) ;
}
...
int l ;
{
   ...
   {
     i++ ;
     csize=(int)s[i] + (int)s[i+1]*256 ; i+=2 ;
     for ( j=0 ; j<csize ; i++,j++,dp++ ) s[i]=lookup[s[i]] ;
   }
   return(dp) ;
}
...
unsigned int xs,ys2 ;
{
   ...
   p = malloc(xs*ys2) ;
   if (p==NULL) cError("","BK Mem Error",1) ;

   if (fp==NULL) cError(fn,"Not Found!",1) ;

   writeMode(0) ;
   actPage(2) ; pClrScr(0) ; actPage(3) ; pClrScr(0) ;
   off = xs*ys2 ;

   fread(p,xs,ys2,fp) ; pSprCp(FP_SEG(p),FP_OFF(p),xs,xs,ys2,0xa800,0,xs/4) ;
   fread(p,xs,ys2,fp) ; pSprCp(FP_SEG(p),FP_OFF(p),xs,xs,ys2,0xa800,off,xs/4) ;
   fclose(fp) ;
   free(p) ;
   return(0) ;
}
...
PLAYER *LP,*RP ;
{
   ...
   bks = bkSize[ENV->Bk] ; moff = bks/4-80 ; off = moff/2 ; t=ENV->t*10 ;
   ...
   for ( ; ; )
   {
     ...
     drawBackground(scr,off,bks) ; drawActor(LP,RP,scr) ;
     if (COMTYPE) viewPage(p) ;
     drawScoreBoard(LP,RP,t) ;

     if (HIT) { HIT=0 ; waitSec(4) ; }

     if (_KE[_F5])
     {
       _KE[_F5]=0 ;
       for ( i=0 ; i<100 ; i++ ) setStAdd(p*0x4000+(i%2)*800) ;
     }

     if ((t%16)==0)
     {
       if (LP->chicken>0) LP->chicken-- ;
       if (RP->chicken>0) RP->chicken-- ;
     }

     if (NewPlayer==0)
     {
       if (setNextAction(LP,RP)==0)  setNextAction(RP,LP) ;
     }
     else NewPlayer++ ;

     if (NewPlayer>20) return(NEWPLAYER) ;

     if (t>0 && NewPlayer==0) t-- ;
     if (t==0) r1End=TIME_END ;

     if (_KE[_ESC]) systemDown() ;
     if (_KE[_F10]) { _KE[_F10]=0 ; return(BREAK) ; }
     if (_KE[_F1])  { _KE[_F1]=0 ; vi = (vi+1)%2 ; }
     if (_KE[_F9])  { _KE[_F9]=0 ; wait = (wait+1)%2 ; }
     if (_KE[_F8])  cError("","mem Test",1) ;

     if (WAITCONST>0) delay(WAITCONST) ;

     newPlayerComeIn(LP) ;
     newPlayerComeIn(RP) ;

     if (r1End) break ;
     if (vi) viewInfo(LP,RP) ;
     if (wait) { viewPage(p) ; KBDWAIT ; }
   }
   offMusic() ;
...
PLAYER *PL ;
{
   ...
   if (PL->ctrl==HUMAN || NewPlayer) return(0) ;

   K = PL->K ;
   if (_KS[K->x]||_KS[K->y]||_KS[K->z]||_KS[K->a]||_KS[K->b]||_KS[K->c])
   {
     PL->ctrl=HUMAN ; PL->pl=NOTSELECT ; NewPlayer=1 ;
   }
}
...
int off,bks,t ;
{
   ...
   fadeOut() ;
   p=0 ; actPage(p) ;
   drawBackground(0,off,bks)  ; drawActor(LP,RP,0) ; drawScoreBoard(LP,RP,t) ;
   viewPage(p) ; sprintf(tmp,"$ROUND %2d #JU@MAA SAQCU@DA@$ !\0",ENV->mn) ;
   fadeIn() ;
   sayVoice("start.voc") ;
   for ( b=0 ; b<30 ; b++ )
   {
     viewPage(p) ; p=(p+1)%2 ; actPage(p) ;
     setPostition(LP,RP,&off,moff) ;
     drawBackground(0,off,bks) ; drawActor(LP,RP,0) ;
     if (COMTYPE) viewPage(p) ;
     drawScoreBoard(LP,RP,t) ;
...
int off,bks,t,p ;
{
   ...
   if (LP->energe<0) LP->energe=0 ;
   if (RP->energe<0) RP->energe=0 ;
   ...
   for ( b=0 ; b<70 ; b++ )
   {
     if (b==40) sayVoice(vn) ;
     viewPage(p) ; p=(p+1)%2 ; actPage(p) ;
     setPostition(LP,RP,&off,moff) ; scr = (min(LP->y,RP->y)-180)/12 ;
     drawBackground(scr,off,bks) ; drawActor(LP,RP,scr) ;
     if (COMTYPE) viewPage(p) ;
     drawScoreBoard(LP,RP,t) ;
...
int x,y,mirr ;
{
   P->ann=0 ; P->ani=0 ; P->energe=120 ; P->ve=120 ;
   P->x=x ; P->y=y ; P->mirr=mirr ;
   P->x2 = 0 ; P->y2 = 0 ; P->end=0 ; P->chicken = 2 ;
   P->ann2 = END_OF_ACTION ; P->ani2=0 ; P->mirr2 = mirr ;
   P->i = P->i2 = 0 ; P->hit=0 ;
   return(0) ;
}
...
int *off,moff ;
{
   ...
   setPlayerActor(LP,LP->ann,LP->ani,&LP->ar,&LP->i) ; aL = LP->ar ;
   setPlayerActor(RP,RP->ann,RP->ani,&RP->ar,&RP->i) ; aR = RP->ar ;

   xl = LP->x ; yl = LP->y + LP->AR[aL].dy ;
   xr = RP->x ; yr = RP->y + RP->AR[aR].dy ;
   dxl = LP->AR[aL].dx ; dxr = RP->AR[aR].dx ;

   if (LP->mirr) dxl *= (-1) ;
   if (RP->mirr) dxr *= (-1) ;
   appr=0 ; if (abs(xl-xr)>abs(xl+dxl-xr-dxr)) appr=1 ;

   if (LP->hit>0) { LP->hit-- ; dxl=0 ; }
   if (RP->hit>0) { RP->hit-- ; dxr=0 ; }

   toff = *off ; xl += dxl ; xr += dxr ; pull=0 ;
/*
...
*/
   ...
   { xl += dxr ; xr += dxl ; pull=1 ; }
   if (isDamage(LP->i)||isDamage(RP->i)||LP->ann==67||LP->ann==68
       ||RP->ann==67||RP->ann==68) if (r1End==0) pull=1 ;

   if (pull) pullControl(&xl,&xr,toff,moff) ;

   if (xl<LMARGIN && xr<RMARGIN && toff>0    )
      { toff -= ((LMARGIN-4-xl)/8+1) ; xr += (LMARGIN-xl)  ; }
   if (xl>RMARGIN && xr>LMARGIN && toff<moff )
      { toff += ((xl-RMARGIN-4)/8+1) ; xr -= (xl-RMARGIN) ; }
   if (xr<LMARGIN && xl<RMARGIN && toff>0    )
      { toff -= ((LMARGIN-xr-4)/8+1) ; xl += (LMARGIN-xr)  ; }
   if (xr>RMARGIN && xl>LMARGIN && toff<moff )
      { toff += ((xr-RMARGIN-4)/8+1) ; xl -= (xr-RMARGIN) ; }

   if (xl<LMARGIN)  xl=LMARGIN  ; if (xl>RMARGIN) xl=RMARGIN ;
   if (xr<LMARGIN)  xr=LMARGIN  ; if (xr>RMARGIN) xr=RMARGIN ;

   if (toff<0) toff=0 ; if (toff>moff) toff=moff ;

   *off = toff ;
   LP->x = xl ; LP->y = yl ; RP->x = xr ; RP->y = yr ;

   LP->ani++ ; RP->ani++ ;
   ansizeL = LP->AN->n[LP->ann] ; ansizeR = RP->AN->n[RP->ann] ;

   if (isFork(LP->i)) creatTempAction(LP) ;
   if (isFork(RP->i)) creatTempAction(RP) ;

   if (LP->ani>ansizeL-2)
   {
     an = LP->AN->Ap[LP->ann] + LP->ani ;
     LP->ann = LP->AN->As[an] ; LP->ani=0 ;
     if (r1End) setWinnerOrLoserAction(LP,RP) ;
   }
   if (RP->ani>ansizeR-2)
   {
     an = RP->AN->Ap[RP->ann] + RP->ani ;
     RP->ann = RP->AN->As[an] ; RP->ani=0 ;
     if (r1End) setWinnerOrLoserAction(RP,LP) ;
   }
...
PLAYER *RP ;
{
   ...
   if (isRAttack(RP->i2)) { RP->ani++ ; return(0) ; }
   an = RP->AN->Ap[RP->ann] + RP->ani ;
   RP->ann2 = RP->AN->As[an] ; RP->ani2=0 ;
   RP->ani++ ;
   RP->mirr2=RP->mirr ; RP->x2 = RP->x ; RP->y2 = RP->y ;
   return(0) ;
}
...
int *xl,*xr,off,moff ;
{
   ...
   if ((*xl)<LMARGIN && off<=0   ) (*xr) += (LMARGIN-(*xl)) ;
   if ((*xl)>RMARGIN && off>=moff) (*xr) -= ((*xl)-RMARGIN) ;
   if ((*xr)<LMARGIN && off<=0   ) (*xl) += (LMARGIN-(*xr)) ;
   if ((*xr)>RMARGIN && off>=moff) (*xl) -= ((*xr)-RMARGIN) ;
   return(0) ;
}
...
PLAYER *PL,*PR ;
{
   ...
   if (r1End==KO_END)
   {
     if (PL->energe>0) setAction(PL,WINNER_ACTION+random(2)) ;
            else       setAction(PL,LOSER_ACTION) ;
   }

   if (r1End==TIME_END)
   {
     setAction(PL,TIME_LOSER_ACTION) ;
     if (PL->energe>PR->energe) setAction(PL,WINNER_ACTION+random(2)) ;
     if (PL->energe<PR->energe) setAction(PL,TIME_LOSER_ACTION) ;
   }
...
PLAYER *LP ;
{
   ...
   if (LP->ann2==END_OF_ACTION) return(0) ;

   setPlayerActor(LP,LP->ann2,LP->ani2,&LP->ar2,&LP->i2) ; aL = LP->ar2 ;
   dxl = LP->AR[aL].dx ; if (LP->mirr2) dxl *= (-1) ;
   LP->x2 += dxl ; LP->y2 += LP->AR[aL].dy ;
...
PLAYER *LP,*RP ;
{
   ...
   switch(RP->ann)
   {
     case 0 : case 2 : case 3 : case 5 : case 8: case 6 : return(1) ;
   }
...
int scr,off,xs ;
{
   ...
   writeMode(1) ; pImgCpLU(0xa800,off,xs/4,80,168+scr,_PSEG[32-scr],0,80) ;
   writeMode(0) ;
   return(0) ;
}
...
int scr ;
{
   ...
   xl=LP->x  ; yl=LP->y-scr ; xr=RP->x ; yr=RP->y-scr ;
   sL=LP->AR[LP->ar].sp ; sR=RP->AR[RP->ar].sp ;
   ...
   {
     off=0 ; if (isUpSideDown(RP->i)) off=2 ;
     pPutRLESprMode(xr,yr,&RP->Spr[sR],RP->Img,RP->mirr+off) ;
     off=0 ; if (isUpSideDown(LP->i)) off=2 ;
   ...
   drawTempActor(LP) ; drawTempActor(RP) ;
   return(0) ;
}
...
int t ;
{
   ...
   if (LP->ve>LP->energe) LP->ve-- ;
   if (RP->ve>RP->energe) RP->ve-- ;

   if (RP->ve>0) pBar2(168,21,168+RP->ve,29,_R5|_G7) ;
   if (LP->ve>0) pBar2(152-LP->ve,21,152,29,_R5|_G7) ;

   pBar2(152,20,168,30,15) ;
   c=255 ; if (LP->energe<40 || RP->energe<40) c=(t%2)*8+247 ;
   fPText(152,18,"$KO",0,c,255) ;
   fPText(30,31,pName[LP->pl],0,255,255)  ;
   fPText(228,31,pName[RP->pl],0,255,255) ;
   sprintf(tmp,"$%3d\0",t/10) ;
   if (t>150 || (t%2) || r1End) fPText(148,31,tmp,0,200,200) ;
   sprintf(tmp,"$%2d\0",LP->win)  ; fPText(8,18,tmp,0,255,255) ;
   sprintf(tmp,"$%-2d\0",RP->win) ; fPText(292,18,tmp,0,255,255) ;
   if (NewPlayer)
   {
...
PLAYER *MP,*EP ;
{
   ...
   if (r1End) return(0) ;
   if (!_AK(EP->i)) flushKey(EP->K) ;
   if (!_AK(MP->i)) flushKey(MP->K) ;

   if (isRAttack(EP->i) && !isFullDefence(MP->i))
      if (damageCheck(MP,EP)) { return(0) ; }
   if (isRAttack(EP->i2) && !isFullDefence(MP->i))
      if (waveDamageCheck(MP,EP)) {  return(0) ; }

   if (_SK(MP->i)) if (specialAttack(MP)) return(0) ;
   if (_MK(MP->i)) movePlayer(MP,EP) ;
   if (_AK(MP->i))
   {
     r=combAttack(MP,EP) ; if (r) return(r) ;
     if (normalAttack(MP,EP)) return(0) ;
   }
   return(0) ;
}
...
PLAYER *MP,*EP ;
{
   if (isOverLap(MP,MP->AR[MP->ar].sp,MP->x,MP->y,MP->mirr,
                 EP,EP->AR[EP->ar].sp,EP->x,EP->y,EP->mirr)==0) return(0) ;
   if (isDefence(MP->i) && (((EP->i&0x0c)>>2)!=(MP->i&0x3)))
   {
     setAction(MP,REAL_DEFENCE_ACTION+MP->ann) ;
     if (EP->ann>40) MP->energe -= 2 ;
     EP->hit=3 ; sayVoice("defence.voc") ;
     return(1) ;
   }
...
PLAYER *MP,*EP ;
{
   ...
   if (isRAttack(MP->i2))
     ...
     {
       EP->ann2 = WAVE_FX_ACTION ; EP->ani2=0 ;
       MP->ann2 = WAVE_FX_ACTION ; MP->ani2=0 ;
       sayVoice("waveFx.voc") ;
       return(1) ;
     }

   if (isOverLap(MP,MP->AR[MP->ar].sp,MP->x,MP->y,MP->mirr,
                 EP,EP->AR[EP->ar2].sp,EP->x2,EP->y2,EP->mirr2)==0) return(0) ;

   if (isDefence(MP->i))
     {
       EP->ann2 = WAVE_FX_ACTION ; EP->ani2=0 ; MP->energe -= 2 ;
       setAction(MP,REAL_DEFENCE_ACTION+MP->ann) ;
...
int da,power,flag ;
{
   ...
   tmp[1]=power%10+'0' ;
   if (flag==0) sayVoice(tmp) ;

   sc    = (EP->power - MP->resis + 120)/30 ;

   MP->energe -= (1 + power*sc) ;
   MP->chicken += power ;
   if (flag) MP->chicken=2 ;
   if (MP->chicken>6 && MP->y==180 ) { da=CHICKEN_DAMAGE ; MP->chicken=0 ; }
   if (MP->energe<=0)
   { r1End=KO_END ; sayVoice("ko.voc") ; if (flag==0) da = KO_DAMAGE ; }
   if (flag==0)
   {
      if (MP->y<150) { i=getIndex(MP) ; replaceAction(MP,da,4,i,0x3) ; }
        else         { MP->y=180 ; setAction(MP,da) ; }
   }
   else setAction(MP,da) ;
   return(0) ;
}
...
//   printf("\n\n (%3d,%3d) sL=%3d  (%3d,%3d) sR=%3d\n",xl0,yl0,sL,xr0,yr0,sR) ;
   ...
   for ( i=0 ; i<ln ; i++ )
    {
      ilp=SprM[sL].i[i] ; 
      yl1=yl0+SprM[sL].yo[i] ; yl2=yl1+ImgM[ilp].ys-1 ;
      if (yl0<180) yl2=yl1+((int)ImgM[ilp].ys*2)/3-1 ;
      xl1=xl0+SprM[sL].xo[i] ; if (lm) xl1=xl0-SprM[sL].xo[i]-SprM[sL].xs ;
      xl2=xl1+ImgM[ilp].xs-1 ;

      irp=SprE[sR].i[j] ;  t1=SprE[sR].xo[0] ; t2=SprE[sR].yo[0] ;
      yr1=yr0+SprE[sR].yh1+t2 ; yr2=yr0+SprE[sR].yh2+t2 ;
      xr1=xr0+SprE[sR].xh1+t1 ; if (rm) xr1=xr0-SprE[sR].xh1-t1 ;
      xr2=xr0+SprE[sR].xh2+t1 ; if (rm) xr2=xr0-SprE[sR].xh2-t1 ;
      t1=min(xr1,xr2) ; t2=max(xr1,xr2) ; xr1=t1 ; xr2=t2 ;
      t1=min(yr1,yr2) ; t2=max(yr1,yr2) ; yr1=t1 ; yr2=t2 ;
...
PLAYER *PL ;
{
   ...
   if ((code=getSpecialCode(PL->KR,PL->KB,PL->ctrl,PL->san))==0) return(0) ;
   PL->KB->b[0] = PL->KB->p = 0 ; flushKey(PL->K) ;
   PL->scr += 100 ;
   setAction(PL,code) ;
   return(1) ;
}
...
char san ;
{
   ...
   if (ctrl==COMPU)
   {
     if (random(10)==0)
     {
       i=random(san) ;  sayVoice(KR[i].vfn) ;
       return(KR[i].code) ;
     }
     return(0) ;
   }
...
char *str1,*str2 ;
{
   ...
   for ( i=0 ; str1[i]!='\0' ; i++ )
   {
     if (islower(str1[i])) str2[i]=tolower(str2[i]) ;
...
PLAYER *MP,*EP ;
{
   ...
   if (ak==0) return(0) ; lc=0 ;
   if (MP->y==180 && EP->y==180 && abs(MP->x - EP->x)<MP->nr) lc=1 ;
   if (MP->y<120 && EP->y<120 && abs(MP->x - EP->x)<MP->nr
       && abs(MP->y - EP->y)<45) lc=2 ;
   if (MP->y==180 && EP->y==180 && abs(MP->x - EP->x)>MP->nr
       && abs(MP->x - EP->x)<MP->nr+15) lc=3 ;
   if (lc==0) return(0) ;

   for ( i=0 ; MP->CR[i].lc<100 ; i++ )
   {
    clc=MP->CR[i].lc % 10 ; dm=MP->CR[i].lc/10 ;
    if (dm==0) dm=3 ;
    if (lc==clc && kc==MP->CR[i].kc)
    {
      if (clc==2) { EP->y=105 ; MP->y=105 ; }
      if (MP->CR[i].dann==0) { setAction(MP,MP->CR[i].ann) ; return(1) ; }
      xmin=min(MP->x,EP->x) ; xmax=max(MP->x,EP->x) ;
      if (!isMove(EP->i)) break ;
      switch(MP->CR[i].mc)
      {
        case 0 : if (EP->mirr) EP->x=MP->x+20 ;
                     else      EP->x=MP->x-20 ;
                     break ;
        case 1 : MP->x=xmin ; EP->x=xmin+20 ; MP->mirr=0 ; EP->mirr=1 ; break ;
        case 2 : MP->x=xmax ; EP->x=xmax-20 ; MP->mirr=1 ; EP->mirr=0 ; break ;
        case 3 : MP->x=xmin ; EP->x=xmin+20 ; MP->mirr=1 ; EP->mirr=1 ; break ;
        case 4 : MP->x=xmax ; EP->x=xmax-20 ; MP->mirr=0 ; EP->mirr=0 ; break ;
      }
...
PLAYER *MP,*EP ;
{
   ...
   if (ac==0) return(0) ;
   nr = 0 ;
   if (abs(MP->x - EP->x)<MP->nr && EP->y==180) nr=1 ;
   switch (MP->ann)
   {
      case 2 :
      case 5 :
      case 8 : mc=4 ; nr=0 ; break ;
      case 1 : mc=2 ; nr=0 ; break ;
      case 4 :
      case 7 : mc=3 ; nr=0 ; break ;
      default: mc=0 ; break ;
   }
   an=(nr+mc)*6+ac+10 ; tmp[1]=an/10+'0' ; tmp[2]=an%10+'0' ;
   sayVoice(tmp) ;
   if (mc!=2 & mc!=3) setAction(MP,(nr+mc)*6+ac+10) ;
       else           replaceAction(MP,mc*6+ac+10,MP->ann,MP->ani,0)  ;
   flushKey(MP->K) ;
   return(1) ;
}
...
int ac,ann,ani,mask ;
{
   ...
   sanp = PL->AN->Ap[ann] ; sans = PL->AN->n[ann]  ;
   tanp = PL->AN->Ap[DUMY_ACTION] ; tar0 = PL->AN->As[tanp] ;
   ranp = PL->AN->Ap[ac] ; rans = PL->AN->n[ac] ; rans-- ;
   ...
   for ( i=0,k=0 ; i<rans ; i++ )
   {
     ...
     tarp = tar0+k ;
     if (isFork(PL->AR[rarp].i)) { i++ ; continue ; }
     PL->AR[tarp].sp = PL->AR[rarp].sp ;
     PL->AR[tarp].i  = PL->AR[rarp].i|mask ;
     k++ ;
   }

   PL->AN->n[DUMY_ACTION] = j+1 ; PL->AN->As[tanp+j]=0 ;
   PL->ann=DUMY_ACTION ; PL->ani=0 ;
   return(0) ;
}
...
PLAYER *PL ;
{
   ...
   anp = PL->AN->Ap[4] ; ans=PL->AN->n[4] ;
   for ( i=0 ; i<ans ; i++ )
   {
     ar = PL->AN->As[anp+i] ;
     y += PL->AR[ar].dy ;
     if (abs(y-PL->y)<20) break ;
   }
   PL->y = y - PL->AR[ar].dy ;
   return(i) ;
}
...
int ctrl ;
{
   if (ctrl==COMPU)
   {
     if (random(20)==0) return(random(6)+1) ;
     return(0) ;
   }
   if (_KE[K->x]) return( 1) ;
   if (_KE[K->y]) return( 2) ;
...
PLAYER *PL,*EP ;
{
   ...
   if (PL->x < EP->x) { PL->mirr=0 ; PL->key2asc[1]='b' ; PL->key2asc[2]='f' ; }
   if (PL->x > EP->x) { PL->mirr=1 ; PL->key2asc[1]='f' ; PL->key2asc[2]='b' ; }

   mc=getMoveCode(PL->K,PL->mirr,PL->ctrl) ; defence=0 ;
   if (isAttack(EP->i)  && abs(PL->x-EP->x)<130) defence=1 ;
   if (isAttack(EP->i2) && abs(PL->x-EP->x2)<130) defence=1 ;

   if (defence)
   {
     if (mc==3) { setAction(PL,HEAD_DEFENCE_ACTION) ; return(0) ; }
     if (mc==5) { setAction(PL,FOOT_DEFENCE_ACTION) ; return(0) ; }
   }
   if (PL->ann!=mc) setAction(PL,mc) ;
   return(0) ;
}
...
int mirr,ctrl ;
{
   ...
   if (ctrl==COMPU)
   {
     if (random(20)==0) return(random(9)) ;
     return(0) ;
   }
...
PLAYER *PL ;
{
   ...
   for ( n=0 ; ; n++ )
   {
     ...
     PCR[n].lc=lc ; PCR[n].kc=kc ;
     PCR[n].ann=ann ; PCR[n].dann=dann ; PCR[n].mc=mc ;
     if (PCR[n].lc==255) break ;
   }
...
ACTOR  *AR ;
{
   ...
   for ( n=0 ; ; )
   {
     fscanf(fp,"%d",&code) ; if (code==(-1)) break ;
     AN->Ap[code]=n ;
     for ( k=0 ; ; n++,k++ )
     {
       fscanf(fp,"%d",&s) ; if (s==(-1)) break ;
       AN->As[n]=s ;
     }
     AN->n[code]=k ;
   }
   ...
   for ( n=0 ; ; n++ )
   {
     if (fscanf(fp,"%d %d %d %d %s",&code,&s,&dx,&dy,info)!=5) break ;
     AR[code].sp=s ; AR[code].dx=dx ; AR[code].dy=dy ;
