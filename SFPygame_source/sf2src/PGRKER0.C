...
**/
void pPutImgMode(x,y,p,xs,ys,m)
int x,y,xs,ys ;
unsigned char far *p  ;
int m ;
{
...
**/
void imgHFill(seg,off,xs,color)
unsigned int seg,off,xs ;
unsigned char color ;
{
   asm {
     push ds
...
/**
   ...
   ÏiE ¡¡—aµA¬á ®»¢¬å·i ‹aŸ¥”a.
**/
void pImgVFill(seg,off,scs,ys,color)
unsigned int seg,off,scs,ys ;
unsigned char color ;
{
   asm {
     push ds
...
**/
void pImgHFill(seg,x1,x2,color)
unsigned int seg,x1,x2,color ;
{
   unsigned int off2 ;
...
**/
void pImgFill(seg,x1,scs,x2,ys,color)
unsigned int seg,x1,x2,color ;
{
   unsigned int off2 ;
